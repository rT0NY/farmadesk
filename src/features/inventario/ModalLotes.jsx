...
function ModalEditarLote({ lote, onCerrar, onGuardar }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/60 animate-fade-in" onClick={onCerrar} />
      <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-modal-in">
...
function ModalCuadrar({ lote, sucursales, onCerrar, onGuardar }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/60 animate-fade-in" onClick={onCerrar} />
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl animate-modal-in">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
...
export default function ModalLotes({ producto, onCerrar, onCambio }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-xl bg-white rounded-none sm:rounded-3xl shadow-2xl h-[100dvh] sm:h-auto sm:max-h-[92dvh] overflow-hidden flex flex-col animate-modal-in">

        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 shadow-md shadow-blue-500/30 flex items-center justify-center"><Package className="w-5 h-5 text-white" /></div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {cargando ? (
            ...
            lotes.map(lote => {
              ...
              const stockSuc = lote.stock_por_sucursal || {}
              return (
                <div key={lote.lote_id} className={cn('bg-white border border-slate-100 shadow-card rounded-2xl overflow-hidden border-l-4', est.borde)}>
                  {/* Cabecera */}
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/50 flex items-center justify-center gap-2">
                    <button onClick={() => setEditarLote(lote)} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all">
                      <Edit2 className="w-3.5 h-3.5" /> Editar
                    </button>
                    <button onClick={() => setCuadrarLote(lote)} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold text-primary-700 bg-primary-50 hover:bg-primary-100 active:scale-95 transition-all">
                      <Scale className="w-3.5 h-3.5" /> Cuadrar
                    </button>
                    <button onClick={() => setEliminarLote(lote)} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 active:scale-95 transition-all">
